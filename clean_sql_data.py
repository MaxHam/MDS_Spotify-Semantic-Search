...
"""
    ...
    for index, row in df.iterrows():
        ...
        if(row[1] != "3N29lMZHMKTVGXUN5aqzl5" and row[1] != "5lFDtgWsjRJu8fPOAyJIAK"):
            sql += sql_instert + f"({row[0]},'{rpQuote(row[1])}','{rpQuote(row[2])}','{rpQuote(row[3])}','{rpQuote(row[4])}',{row[5]},'{rpQuote(row[6])}','{rpQuote(row[7])}',{rpDate(row[8])},'{rpQuote(row[9])}','{rpQuote(row[10])}','{rpQuote(row[11])}','{rpQuote(row[12])}',{row[13]},{row[14]},{row[15]},{row[16]},{row[17]},{row[18]},{row[19]},{row[20]},{row[21]},{row[22]},{row[23]},{row[24]},'{rpQuote(row[25])}');\n"
        if(index % 100 == 0):
            print(index)
