...
def document_query(client, query, limit=10, columns=columns):
    withFilter = {
        "operator": "Or",
        "operands": [
            {
...
def run_benchmark(weaviate_client, query, query_func, limit, benchmark_id, database):
    query_time, result = aggregate_query_time(lambda: query_func(weaviate_client, query=[query], limit=limit))
...
def main(args):
    ...
    result = run_benchmark(weaviate_client=weaviate_client, query=mul_query, query_func=document_query, limit=limit, benchmark_id="BM2", database="document")
    df = pd.concat([df, result], ignore_index=True)
    ...
    df = pd.concat([df, result], ignore_index=True)
    # Measure  query time for document database
    result = run_benchmark(weaviate_client=weaviate_client, query=single_column_query, query_func=document_query, limit=limit, benchmark_id="BM4", database="document")
    df = pd.concat([df, result], ignore_index=True)
